...
function bandEqualizer(audioContext, frequency, gain) {
	...
	/*equalizers.push(
	this.bandEqualizer(audioContext
,65
, sureNumeric(readTextFromlocalStorage('equalizer0'),-10		,8,+10)
	)
	);*/
	var q = 1.0;
			...
			for (var d = 0; d < drum.notes.length; d++) {
				if (drum.notes[d].beat == n) {
					...
					player.queueWaveTable(audioContext
						...
						//,inGain
					, drum.gain, drum.sound, startTime + n16 * beatLen, drum.pitch, 1.75, v);
				}
			}
		}
		for (var i = 0; i < tracks.length; i++) {
			...
			for (var s = 0; s < chord.length; s++) {
				...
				var shift = [{
						...
						pitch : note.shift + track.octave * 12 + note.pitch
					}
				];
				player.queueWaveTable(audioContext
					//, audioContext.destination
					//,inGain
				, track.gain, track.sound, startTime + n16 * beatLen + s * inChordDelay, track.octave * 12 + note.pitch, note.length * beatLen, v, shift);
			}
		}
	}
...
if(note.beat==markBeat){
//console.log(note);
lightSparkle(20,markBeat+0.5,1.5+note.pitch
//,-1*t+1.0,1,track.light,note.length);
}
}
}
}
...
function showEqualizer() {
	...
	for (var i = 0; i < equalizers.length; i++) {
		//console.log(i,equalizers[i].frequency.value,equalizers[i].gain.value);
		var gain = equalizers[i].gain.value;
		if (gain < 0) {
...
function midNoteOn(midiNote) {
	...
	var track = tracks[0];
	midiFindNote(midiNote).envelope = player.queueWaveTable(audioContext, track.gain, track.sound, 0, track.octave * 12 + midiNote - 2 * 12, 99999, 0.75 * track.volumeRatio, 0);
