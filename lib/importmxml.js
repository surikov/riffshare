...
FretChordSheet.prototype.parsePartTrackNum = function (vt, id) {
	...
	for (var i = 0; i < pls.length; i++) {
		...
		if (sp.of('id').value == id) {
			...
			//var unpitched = sp.of('midi-instrument').of('midi-unpitched').value;
...
FretChordSheet.prototype.parsePart = function (vt, track, part) {
	//console.log('part', part.of('id').value);
	...
	for (var i = 0; i < measures.length; i++) {
		...
		if (_fifths.length > 0) {
			_fifths = 1 * _fifths;
			//if (_fifths>0 || _fifths<0) {
			if (_fifths != fifths) {
				fifths = _fifths;
				//negative for flats, positive for sharps
				//console.log(i,'fifths',fifths);
			}
		}
		...
		//console.log('fifths',fifths,minfo.keys);
		if (track > -1) {
...
FretChordSheet.prototype.calcPitch = function (step, octave, accidental, keys,clefoctavechange) {
	//sharp, flat, natural, double - sharp, sharp - sharp, flat - flat, natural - sharp, natural - flat, quarter - flat, quarter - sharp, three - quarters - flat, and three - quarters - sharp
	//var r = 12 * (octave - 1-clefoctavechange);
	var r = 12 * (octave - 1);//-clefoctavechange);
	var stepNum = 0;
	if (step.toUpperCase() == 'C') { r = r + 0; stepNum = 0; }
	if (step.toUpperCase() == 'D') { r = r + 0 + 2; stepNum = 1; }
	if (step.toUpperCase() == 'E') { r = r + 0 + 2 + 2; stepNum = 2; }
	if (step.toUpperCase() == 'F') { r = r + 0 + 2 + 2 + 1; stepNum = 3; }
	if (step.toUpperCase() == 'G') { r = r + 0 + 2 + 2 + 1 + 2; stepNum = 4; }
	if (step.toUpperCase() == 'A') { r = r + 0 + 2 + 2 + 1 + 2 + 2; stepNum = 5; }
	if (step.toUpperCase() == 'B') { r = r + 0 + 2 + 2 + 1 + 2 + 2 + 2; stepNum = 6; }
	if (accidental) {
		if (accidental == 'sharp') {
			r = r + 1;
		}
		...
		for (var i = 0; i < notes.length; i++) {
			if (voice == notes[i].of('voice').value) {
				...
				if (notes[i].has('pitch')) {
					...
					var pitch = this.calcPitch(step, octave, accidental, keys,clefoctavechange);
