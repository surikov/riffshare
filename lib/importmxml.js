...
FretChordSheet.prototype.midi2ins = function (prg) {
	...
	var midi = prg + 1;
	if (midi == 31) { return 0; }
	if (midi >= 25 && midi <= 28) { return 1; }
	if (midi >= 17 && midi <= 24) { return 2; }
	if (midi == 30) { return 3; }
	//
	if (midi >= 33 && midi <= 38) { return 5; }
	if (midi >= 41 && midi <= 88) { return 6; }
	if (midi >= 39 && midi <= 40) { return 7; }
	return r;
};
FretChordSheet.prototype.midi2drum = function (midi) {
	...
	if (midi == '45' || midi == '47' || midi == '48' || midi == '50') { return 3; }
	if (midi == '42' || midi == '44') { return 4; }
	if (midi == '46') { return 5; }
	//
	if (midi == '49' || midi == '36') { return 7; }
	return r;
};
		...
		if (scoreparts[i].of('id').value == partId) {
			...
			for (var n = 0; n < midiinstruments.length; n++) {
				if (midiinstruments[n].of('id').value == insId) {
					return this.midi2drum(midiinstruments[n].of('midi-unpitched').value);
				}
			}
		}
	}
