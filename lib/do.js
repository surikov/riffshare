...
FretChordSheet.prototype.userActionAddNote = function (tone, start192, len192, pitch, shift, string) {
	//string = string|me.stringNumber(me.tones[t].pitch);
	var one = { tone: tone, start192: start192, len192: len192, pitch: pitch, shift: shift, string: string };
	var me = this;
	this.pushAction({
...
FretChordSheet.prototype.userActionDropFrets = function (tone, start192, len192, string) {
	var me = this;
	var notes = me.findFrets(tone, start192, len192, string);

	this.pushAction({
		caption: 'Drop frets ' + tone + '/' + string + '-' + start192 + ':' + len192,
		undo: function () {
			for (var i = 0; i < notes.length; i++) {
				me.addNote(notes[i]);
			}
...
FretChordSheet.prototype.userActionAddNote = function (trackNo,measureNo,start192,duration192) {
	var me = this;
	this.pushAction({
		caption: 'Add note',
		undo: function () {
