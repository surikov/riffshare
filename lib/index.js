...
function tileNoteGroupDown(group, tg, tileLevel) {
	...
	for (var i = 0; i < group.length; i++) {
		...
		var h = (6.6 -( dy + iy - ry)+ group[i].chord[group[i].chord.length - 1] - group[i].chord[0]) * 3 * tileLevel.tapSize;
		/*tileLevel.tileRectangle(tg.g //
		, tg.x + sheetX + (x * 3 + 4.05) * tileLevel.tapSize //
		, tg.y + (y * 3 + 1.1 - 6.6 * 3) * tileLevel.tapSize //
		, 0.4 * tileLevel.tapSize //
		, h //
		, fgColor);*/
		tileLevel.tileRectangle(tg.g
		, tg.x + sheetX + (x * 3 - 0.7) * tileLevel.tapSize 
		, tg.y + (y * 3 + 2.4-3*(group[i].chord[group[i].chord.length - 1] - group[i].chord[0])) * tileLevel.tapSize
		, 0.4 * tileLevel.tapSize
		, h-dy
		, fgColor);
	}
