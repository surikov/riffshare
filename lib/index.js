...
FretChordSheet.prototype.existsNotes = function (track,morder, start192, duration192, pitch) {
	//var notes = [];
				...
				, 'sptPR' + x + 'x' + y, left, top, width, height, function (tg) {
					var s = tg.layer.addSpot(tg.id, tg.x, tg.y, tg.w, tg.h, function () {
						...
						if (me.markNotes.length >= me.options.markNotesCount) {
							...
							var notes = me.existsNotes(track,this.morder, start192, duration192, pitch);
							console.log(notes);
...
FretChordSheet.prototype.__tilePianorollNoteLine = function (tg, tonenote, start192, measureNum) {
	...
	var tx2 = this.findBeatX(measureNum, start192 + tonenote.slides[0].duration192 - 6) + 1.5 * tg.layer.tapSize;
	if (tx2 - tx < 1) {
		tx2 = tx + 1;
	}
	tg.layer.tileLine(tg.g
		...
		, tx2
		, ty + 1.5 * tg.layer.tapSize + tonenote.slides[0].shift * 3 * tg.layer.tapSize
