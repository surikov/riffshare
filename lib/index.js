...
FretChordSheet.prototype.noteExistsAtBeat = function (minfo,beat192) {
			...
			this.layerOctaves.renderGroup(mx + this.margins.sheetLeft + this.options.measureHeader * 3 * this.tiler.tapSize
				...
				, 'sptPR' + x + 'x' + y, left, top, width, height, function (tg) {
					var s = tg.layer.addSpot(tg.id, tg.x, tg.y, tg.w, tg.h, function () {
						var x = me.findMeasureBeat192(me.tiler.clickContentX, this.mx, this.minfo);
						var beat192 = x.quarter * 8 * 6 + x.left6;
						var duration192 = x.duration6;
						var pitch = 59 - Math.floor((me.tiler.clickContentY - me.margins.pianorollTop) / (3 * me.tiler.tapSize));
						//console.log(pitch,beat192,duration192);
						if (me.markNote) {
							//console.log('drop mark', me.markNote);
							me.markNote = null;
						} else {
							//console.log('set mark', minfo, this.morder, beat192, duration192);
							me.markNote = { minfo: minfo, morder: this.morder, beat192: beat192, duration192: duration192 ,pitch:pitch};
						}
					});
					s.minfo = minfo;
					s.mx = tg.x;
							...
							this.layerNotes.renderGroup(mx + this.margins.sheetLeft + this.options.measureHeader * 3 * this.tiler.tapSize
								...
								, 'markNote', left, top, width, height, function (tg) {
									...
									tg.layer.tileCircle(tg.g
										, tg.x + me.markNote.beat192/6* 3 * me.tiler.tapSize+3 * me.tiler.tapSize / 2
...
FretChordSheet.prototype.tileKnob = function (tg, id, x, y, sz, label, action) {
	...
	tg.layer.addSpot(id, tg.x + x, tg.y + y, sz, sz, function () {
		action();
	});
};
