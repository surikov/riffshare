console.log('FretChordSheet v1.45');
...
FretChordSheet.prototype.pitch2staffY = function (pitch, hintOctave, hintStep) {
	...
	return 6 * 7 - 7 * octave - step - 1;
};
FretChordSheet.prototype.tileNoteAltIf = function (pitch, alts, xx, yy, tg) {
	var step = this.note7(pitch);
	var half = pitch % 12;
	var octave = Math.floor(pitch / 12);
	var rePitch = octave * 12 + half + alts[step];
	var diff = pitch - rePitch;
	...
	//console.log(pitch, octave, half, step, rePitch, diff, alts[step]);
};
/*
FretChordSheet.prototype.pitch2step = function (pitch) {
	var d = pitch % 12;
	var n = 0;
	if (d == 0 || d == 1) n = 0;
	if (d == 2 || d == 3) n = 1;
...
FretChordSheet.prototype.step2pitch = function ( step) {
	var p = 0;//octave * 12;
	if (step == 0) { p = p + 0; }
	if (step == 1) { p = p + 2; }
	if (step == 2) { p = p + 4; }
...
FretChordSheet.prototype.pitch2octave = function (pitch) {
							...
							if (o == 0) {
								...
								if (chord) {
									for (var n = 0; n < chord.notes.length; n++) {
										...
										if (note) {
											var yy = me.pitch2staffY(note.pitch, 0, 0);
											var xx = me.options.measureHeader + beat.start192 / 6;
											me.tileNoteHead(xx, yy, tg);
											//var alt=
											me.tileNoteAltIf(note.pitch, alts, xx, yy, tg);
