console.log('FretChordSheet v1.46');
function getUrlVars() {
	var vars = {};
...
FretChordSheet.prototype.init = function () {
	...
	this.tiler.addZoomLevel(3, function (left, top, width, height) {
		//console.log('addZoomLevel 3 accidental');
	...
	this.tiler.addZoomLevel(9, function (left, top, width, height) {
		//console.log('addZoomLevel 9 notes');
	...
	this.tiler.addZoomLevel(50, function (left, top, width, height) {
		//console.log('addZoomLevel 50 measures');
	...
	this.tiler.addZoomLevel(500, function (left, top, width, height) {
		//console.log('addZoomLevel 500 whole song');
...
FretChordSheet.prototype.tileBackground = function (left, top, width, height, lineWidth) {
	...
	this.layerBack.renderGroup(0, 0, w1, this.contentHeight
		, 'bgPane', left, top, width, height, function (tg) {
			...
			/*var x1=530 * tg.layer.tapSize;
			while(x1<w1){
				tg.layer.tileRectangle(tg.g
					, tg.x + x1
					, tg.y 
					, 110 * tg.layer.tapSize
					, tg.h
					, 'rgba(0,0,0,0.1)');
				x1=x1+530 * tg.layer.tapSize;
			}
			x1=0;
			while(x1<w1){
				tg.layer.tileRectangle(tg.g
					...
					, tg.h
					, 'rgba(0,0,0,0.1)');
				x1=x1+190 * tg.layer.tapSize;
			}
			x1=50 * tg.layer.tapSize;
			while(x1<w1){
				tg.layer.tileRectangle(tg.g
					, tg.x + x1
					, tg.y 
...
FretChordSheet.prototype.tileNoteTools = function (note, xx, yy, tg) {
	...
	this.tileKnob(tg, 'noteAlt' + xx + 'x' + yy
		...
		, this.tiler.tapSize, 'Accidental', function () {
...
FretChordSheet.prototype.tileStaffSpot = function (left, top, width, height, lineWidth) {
	...
	var mx = 0;
	var minfo = this.measureInfo(0);
	//console.log('spots',this.tiler.spots);
	for (var x = 0; x <= this.measures.length; x++) {
		var d32 = 0;
			...
			, 'staffMsrSpt' + x, left, top, width, height, function (tg) {
				var s = tg.layer.addSpot(tg.id, tg.x, tg.y, tg.w, tg.h, function () {
					...
					if (me.markNotes.length >= me.options.markNotesCount) {
						...
						//console.log(note, me.markNotes);
						//console.log('add staff',note);
...
FretChordSheet.prototype.tilePianoSpot = function (left, top, width, height, lineWidth) {
	//console.log('tilePianoSpot',left, top, width, height, lineWidth);
	var me = this;
	var mx = 0;
	var minfo = this.measureInfo(0);
	//console.log('spots',this.tiler.spots);
				...
				, 'pianoOctave' + x + 'x' + y, left, top, width, height, function (tg) {
					var s = tg.layer.addSpot(tg.id, tg.x, tg.y, tg.w, tg.h, function () {
						...
						if (me.markNotes.length >= me.options.markNotesCount) {
							...
							//console.log('add pianoroll',note);
							me.userActionAddNote(track, me.markNotes[0].morder, me.markNotes[0].start192, note);
							me.markNotes = [];
							me.shrinkMeasures();
							me.reCalcContentSize();
...
*/
FretChordSheet.prototype.tileSmallNoteNatural = function (x, y, tg) {
	tg.layer.tilePath(tg.g, tg.x + (x * 3 + 0.75) * tg.layer.tapSize
		, tg.y + (y * 3 - 2) * tg.layer.tapSize
...
FretChordSheet.prototype.tileSmallNoteSharp = function (x, y, tg) {
	tg.layer.tilePath(tg.g, tg.x + (x * 3 - 24) * tg.layer.tapSize
		, tg.y + (y * 3 - 132) * tg.layer.tapSize
...
FretChordSheet.prototype.tileSmallNoteFlat = function (x, y, tg) {
	tg.layer.tilePath(tg.g
		, tg.x + (x * 3 - 47) * tg.layer.tapSize
		, tg.y + (y * 3 - 220) * tg.layer.tapSize
		, 0.5 * tg.layer.tapSize //
...
FretChordSheet.prototype.tileNoteAltFlat = function (pitch, alts, xx, yy, tg) {
	...
	var rePitch = octave * 12 + this.note12(step);
	var diff = pitch - rePitch + alts[step];
	//console.log(xx,pitch,rePitch,step,alts[step]);
	if (diff > 0) {
							...
							if (o == 0) {
								...
								if (chord) {
									for (var n = 0; n < chord.notes.length; n++) {
										var note = chord.notes[n];
										if (note) {
											/*var step=me.note7(note.pitch);
											if(note.hintStep>-1){
												console.log('hint',note.hintStep);
											}
											console.log(step,note.pitch);*/
											var yy = me.pitch2staffY(note.pitch);//, 0, 0, !altModeSharp);
											var xx = me.options.measureHeader + beat.start192 / 6;
											me.tileNoteHead(xx, yy, tg);
											//var alt=
											//if (altModeSharp) {
											//me.tileNoteAltSharp(note.pitch, alts, xx, yy, tg);
											//} else {
											//me.tileNoteAltFlat(note.pitch, alts, xx, yy, tg);
											//}
											//if(alt==1){me.tileNoteSharp(xx-1.5,yy,tg);}
											//if(alt==-1){me.tileNoteFlat(xx-1.5,yy,tg);}
										}
									}
								}
							}
