console.log('Edit3way');
function Edit3way() {
	console.log('create Edit3way');
	return this;
}
Edit3way.prototype.init = function () {
	console.log('init Edit3way');

	this.tiler = new TileLevel("contentSVG", 'cntnt');
	this.tiler.translateZ = 6.9;
	this.layerBack = this.tiler.addLayer();
	this.layerBarNumbers = this.tiler.addLayer(true);
	...
	//this.gridColor = '#666';
	this.colors = { base: '#000', grid: '#666', barCounter: '#600' };
	this.barCount = 16;
	this.barSize = 32;
	...
	this.tiler.addZoomLevel(1.3, function (left, top, width, height) {
		...
		me.tileStaff(left, top, width, height, me.tiler, me.layerBack, lineWidth);
		me.tileBars(left, top, width, height, me.tiler, me.layerBack, lineWidth);
		me.tileSmallMeasureNumbers(left, top, width, height, me.tiler, me.layerBarNumbers, lineWidth);
	});

	this.tiler.addZoomLevel(7, function (left, top, width, height) {
		//console.log('render', 7, this.tiler.translateZ);
		var lineWidth = 0.1 * me.tiler.tapSize;
		me.tileStaff(left, top, width, height, me.tiler, me.layerBack, lineWidth);
		for (var n = 0; n < me.barCount; n++) {
			me.tileMeasureGrid(left, top, width, height, me.tiler, me.layerBack, lineWidth, n);
		}
		me.tileBars(left, top, width, height, me.tiler, me.layerBack, lineWidth);
		me.tileBigMeasureNumbers(left, top, width, height, me.tiler, me.layerBarNumbers, lineWidth);
	});
	this.tiler.addZoomLevel(15, function (left, top, width, height) {
		//console.log('render', 15, this.tiler.translateZ);
...
Edit3way.prototype.tileStaff = function (left, top, width, height, tileLevel, layer, lineWidth) {
	var tg = tileLevel.tileGroup(0, 0, this.contentWidth, this.contentHeight, 'bgrec', layer, left, top, width, height);
	if (tg) {
...
*/
		...
		tileLevel.tileRectangle(tg.g, this.sheetX, this.sheetY + 6 * 0 * tileLevel.tapSize, this.contentWidth, lineWidth, this.colors.base);
		tileLevel.tileRectangle(tg.g, this.sheetX, this.sheetY + 6 * 1 * tileLevel.tapSize, this.contentWidth, lineWidth, this.colors.base);
		tileLevel.tileRectangle(tg.g, this.sheetX, this.sheetY + 6 * 2 * tileLevel.tapSize, this.contentWidth, lineWidth, this.colors.base);
		tileLevel.tileRectangle(tg.g, this.sheetX, this.sheetY + 6 * 3 * tileLevel.tapSize, this.contentWidth, lineWidth, this.colors.base);
		tileLevel.tileRectangle(tg.g, this.sheetX, this.sheetY + 6 * 4 * tileLevel.tapSize, this.contentWidth, lineWidth, this.colors.base);
		/*tileLevel.tiler.tilerectangle(tg.g, sheetX, sheetY + 6 * 1 * tileLevel.tapSize, contentWidth, lineWidth, fgColor);
		tileLevel.tiler.tilerectangle(tg.g, sheetX, sheetY + 6 * 2 * tileLevel.tapSize, contentWidth, lineWidth, fgColor);
		tileLevel.tiler.tilerectangle(tg.g, sheetX, sheetY + 6 * 3 * tileLevel.tapSize, contentWidth, lineWidth, fgColor);
		tileLevel.tiler.tilerectangle(tg.g, sheetX, sheetY + 6 * 4 * tileLevel.tapSize, contentWidth, lineWidth, fgColor);*/
	}
}
Edit3way.prototype.tileBars = function (left, top, width, height, tileLevel, layer, lineWidth) {
	for (var i = 0; i < this.barCount; i++) {
		var xx = this.sheetX + i * 3 * this.barSize * tileLevel.tapSize;
		var tg = tileLevel.tileGroup(0, 0, this.contentWidth, this.contentHeight, 'brLine' + i, layer, left, top, width, height);
		if (tg) {
			tileLevel.tileRectangle(tg.g, xx, this.sheetY, lineWidth, 2 * 3 * 4 * tileLevel.tapSize, this.colors.base);
		}
	}
}
Edit3way.prototype.tileMeasureGrid = function (left, top, width, height, tileLevel, layer, lineWidth, n) {
	var xx = this.sheetX + n * 3 * this.barSize * tileLevel.tapSize;
	var tg = tileLevel.tileGroup(0, 0, this.contentWidth, this.contentHeight, 'brGrid' + n, layer, left, top, width, height);
	if (tg) {
		for (var x = 0; x < this.barSize; x++) {
			tileLevel.tileRectangle(tg.g, xx + x * 3 * tileLevel.tapSize, 0, lineWidth / 7, this.contentHeight, this.colors.grid);
		}
		for (var y = 0; y < this.contentHeight; y = y + 3 * tileLevel.tapSize) {
			tileLevel.tileRectangle(tg.g, xx, y, 3 * this.barSize * tileLevel.tapSize, lineWidth / 7, this.colors.grid);
			//console.log(y);
		}
	}
}

Edit3way.prototype.tileBigMeasureNumbers = function (left, top, width, height, tileLevel, layer) {

	for (var i = 0; i < this.barCount; i++) {
		var xx = this.sheetX + i * 3 * this.barSize * tileLevel.tapSize;
		var ww = xx + 3 * this.barSize * tileLevel.tapSize;
		var tg = tileLevel.tileGroupY(xx, ww, 'brNum' + i, layer, left, width);
		//console.log('tileBigMeasureNumbers',i,'/', xx,0,ww, this.contentHeight,'/',left, top, width, height,tg);
		if (tg) {
			tileLevel.tileText(tg.g, xx + tileLevel.tapSize, 5 * tileLevel.tapSize, 6 * tileLevel.tapSize, '' + (1 + i), this.colors.barCounter);

		}
	}
}
Edit3way.prototype.tileSmallMeasureNumbers = function (left, top, width, height, tileLevel, layer) {
	for (var i = 0; i < this.barCount; i++) {
		var xx = this.sheetX + i * 3 * this.barSize * tileLevel.tapSize;
		var tg = tileLevel.tileGroupY(0, this.contentWidth, 'brNum' + i, layer, left, width);
		if (tg) {
			tileLevel.tileText(tg.g, xx + tileLevel.tapSize, 2 * tileLevel.tapSize, 2 * tileLevel.tapSize, '' + (1 + i), this.colors.barCounter);
		}
	}
}
Edit3way.prototype.tileNoteChordHead = function (x, ys, tg, tileLevel) {
	for (var i = 0; i < ys.length; i++) {
		tileNoteHead(x, ys[i], tg, tileLevel);
	}
}

Edit3way.prototype.tileNoteHead = function (x, y, tg, tileLevel) {
...
Edit3way.prototype.tileNoteSharp = function (x, y, tg, tileLevel) {
...
Edit3way.prototype.tileNoteFlat = function (x, y, tg, tileLevel) {
...
Edit3way.prototype.tileNoteNatural = function (x, y, tg, tileLevel) {
...
Edit3way.prototype.tileNoteStemUp = function (x, y, tg, tileLevel) { //, yShift, hShift) {
	//var dy = yShift || 0;
	//var dh = hShift || 0;
	tileLevel.this.tilerectangle(tg.g, tg.x + sheetX + (x * 3 + 4.05) * tileLevel.tapSize, tg.y + (y * 3 + 1.1 - 6.6 * 3) * tileLevel.tapSize // - dy
		, 0.4 * tileLevel.tapSize, 6.6 * 3 * tileLevel.tapSize //+ dy + dh
		, this.colors.base);
}
Edit3way.prototype.tileNoteStemDown = function (x, y, tg, tileLevel) {
	tileLevel.this.tilerectangle(tg.g, tg.x + sheetX + (x * 3 - 0.7) * tileLevel.tapSize, tg.y + (y * 3 + 2.4) * tileLevel.tapSize, 0.4 * tileLevel.tapSize, 6.6 * 3 * tileLevel.tapSize, this.colors.base);
}
Edit3way.prototype.tileNoteSingle4up = function (x, y, tg, tileLevel) {
	tileNoteHead(x, y, tg, tileLevel);
	tileNoteStemUp(x, y, tg, tileLevel);
}
Edit3way.prototype.tileNoteSingle4down = function (x, y, tg, tileLevel) {
	tileNoteHead(x, y, tg, tileLevel);
	tileNoteStemDown(x, y, tg, tileLevel);
}

Edit3way.prototype.tileNoteSingle8up = function (x, y, tg, tileLevel) {
	tileNoteHead(x, y, tg, tileLevel);
	tileNoteStemUp(x, y, tg, tileLevel);
...
Edit3way.prototype.tileNoteSingle8down = function (x, y, tg, tileLevel) {
	tileNoteHead(x, y, tg, tileLevel);
	tileNoteStemDown(x, y, tg, tileLevel);
...
Edit3way.prototype.tileNoteSingle16down = function (x, y, tg, tileLevel) {
	tileNoteHead(x, y, tg, tileLevel);
	tileNoteStemDown(x, y, tg, tileLevel);
	tileLevel.tilePath(tg.g, tg.x + sheetX + (x * 3 - 0.5) * tileLevel.tapSize, tg.y + (y * 3 + 12.8 + 9.4) * tileLevel.tapSize, 0.05 * tileLevel.tapSize //
...
Edit3way.prototype.tileNoteSingle16up = function (x, y, tg, tileLevel) {
	tileNoteHead(x, y, tg, tileLevel);
	tileNoteStemUp(x, y, tg, tileLevel);
	tileLevel.tilePath(tg.g, tg.x + sheetX + (x * 3 + 4.2) * tileLevel.tapSize, tg.y + (y * 3 - 9.6 - 9) * tileLevel.tapSize, 0.05 * tileLevel.tapSize //
...
Edit3way.prototype.tileNoteBeam = function (fromX, fromY, toX, toY, size, tg, tileLevel) {
...
Edit3way.prototype.tileNoteTest = function (tg, tileLevel) {
	var x1 = 0;
	var y1 = 5;
	var x2 = 8;
	var y2 = 8;
...
Edit3way.prototype.arrayMax = function (array) {
	return array.reduce(function (a, b) {
		return Math.max(a, b);
	});
}

Edit3way.prototype.arrayMin = function (array) {
	return array.reduce(function (a, b) {
		return Math.min(a, b);
	});
}
Edit3way.prototype.tileNoteGroupUp = function (group, tg, tileLevel) {
	...
	for (var i = 0; i < group.length; i++) {
		...
		tileLevel.this.tilerectangle(tg.g, tg.x + sheetX + (x * 3 + 4.05) * tileLevel.tapSize, tg.y + (y * 3 + 1.1 - 6.6 * 3) * tileLevel.tapSize, 0.4 * tileLevel.tapSize, h //6.6 * 3 * tileLevel.tapSize
			, this.colors.base);
	}
...
Edit3way.prototype.tileNoteGroupDown = function (group, tg, tileLevel) {
	for (var i = 0; i < group.length; i++) {
		tileNoteChordHead(group[i].step32, group[i].chord, tg, tileLevel);
	}
	var x1 = group[0].step32;
	...
	for (var i = 0; i < group.length; i++) {
		...
		var h = (6.6 - (dy + iy - ry) + group[i].chord[group[i].chord.length - 1] - group[i].chord[0]) * 3 * tileLevel.tapSize;
		/*tileLevel.this.tilerectangle(tg.g //, tg.x + sheetX + (x * 3 + 4.05) * tileLevel.tapSize //, tg.y + (y * 3 + 1.1 - 6.6 * 3) * tileLevel.tapSize //, 0.4 * tileLevel.tapSize //, h //, fgColor);*/
		tileLevel.this.tilerectangle(tg.g, tg.x + sheetX + (x * 3 - 0.7) * tileLevel.tapSize, tg.y + (y * 3 + 2.4 - 3 * (group[i].chord[group[i].chord.length - 1] - group[i].chord[0])) * tileLevel.tapSize, 0.4 * tileLevel.tapSize, h - dy, this.colors.base);
