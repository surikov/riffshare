...
FretChordSheet.prototype.loadState = function (prefix) {
	try{
		...
		this.options.feel=sureNumeric(p.feel,0,0,3);
...
FretChordSheet.prototype.init = function () {
	...
	this.margins = {
		...
		, sheetTop: 63 * 3 * this.tiler.tapSize
		, pianorollTop: 1 * 3 * this.tiler.tapSize
		, drumsTop: 115 * 3 * this.tiler.tapSize
		, fretTop: 106 * 3 * this.tiler.tapSize
	};
	//this.feels={straight:0,swing:1,triplet:2};
	this.feelNames=['straight 16th','swing 16th','triplet 8th','straight 32th'];
	this.options = {
		measureLen: 32
		, measureCount: 16
		, feel: 0
	...
	this.colors = {
		base: '#000'
		, grid: 'rgba(127,127,127,0.3)'
		, barCounter: 'rgba(0,0,0,0.1)'
		, staff: '#000'
	...
	this.tiler.addZoomLevel(1.3, function (left, top, width, height) {
		...
		me.tileMenu(left, top, width, height, me.layerBack, lineWidth);
	});
	...
	this.tiler.addZoomLevel(9, function (left, top, width, height) {
		...
		//me.tileTest(left, top, width, height, me.layerTest, lineWidth);
		me.tileBarNumbers(9 * me.tiler.tapSize, left, top, width, height, me.layerBarNumbers, lineWidth);
		me.layerOctaveNumbers.lockX = me.tiler.svg.clientWidth - 9 * me.tiler.tapSize;
		me.tileOctaveNumbers(20 * me.tiler.tapSize, left, top, width, height, me.layerOctaveNumbers, lineWidth);
		me.tileDrumGrid(left, top, width, height, me.layerBack, lineWidth);
		me.tilFretGrid(left, top, width, height, me.layerBack, lineWidth);
		me.tilePianorollKeys(left, top, width, height, me.layerBack, lineWidth);
