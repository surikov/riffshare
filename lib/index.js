...
function tileNoteStemUp(x, y, tg, tileLevel) {
	tileLevel.tileRectangle(tg.g, tg.x + sheetX + (x * 3 + 4.05) * tileLevel.tapSize, tg.y + (y * 3 + 1.1 - 6.6 * 3) * tileLevel.tapSize, 0.4 * tileLevel.tapSize, 6.6 * 3 * tileLevel.tapSize, fgColor);
}
...
function tileNoteGroupUp(start32, group, tg, tileLevel) {
	console.log(group);
	var s32 = start32;
	for (var i = 0; i < group.length; i++) {
		tileNoteChordHead(s32, group[i].chord, tg, tileLevel);
		tileNoteStemUp(s32, group[i].chord[0], tg, tileLevel);
		if (i < group.length - 1) {
			s32 = s32 + group[i].duration32;
		}
		/*
	...
	tileNoteBeam(tg.x + (10.3 + 3 * x1) * tileLevel.tapSize, tg.y + (3 * y1 - 9.7-9) * tileLevel.tapSize, tg.x + (10.1 + 3 * x2) * tileLevel.tapSize, tg.y + (3 * y2 - 9.7-9) * tileLevel.tapSize, 1.99 * tileLevel.tapSize, tg, tileLevel);
