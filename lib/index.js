			...
			this.layerOctaves.renderGroup(mx + this.margins.sheetLeft + this.options.measureHeader * 3 * this.tiler.tapSize
				...
				, 'sptPR' + x + 'x' + y, left, top, width, height, function (tg) {
					var s = tg.layer.addSpot(tg.id, tg.x, tg.y, tg.w, tg.h, function () {
						...
						var x = me.findMeasureBeat192(me.tiler.clickContentX, this.mx, measure);//this.minfo);
						var start192 = x.quarter * 8 * 6 + x.left6;
						var full192=me.findBeatDistance(0,0,this.morder,start192);
						var duration192 = x.duration6;
						...
						if (me.markNote) {
							...
							note.pitch = me.markNote.pitch;
							measure = me.measureInfo(me.markNote.morder);
							note.shift = me.markNote.pitch - pitch;
							var at192 = me.markNote.start192;
							var mark192=me.findBeatDistance(0,0,me.markNote.morder,me.markNote.start192);
							note.duration192 = full192 - mark192 + duration192;
							if (full192 < mark192) {
								note.duration192 = mark192 - full192 + me.markNote.duration192;
								note.pitch = pitch;
								note.shift = pitch - me.markNote.pitch;
								at192 = start192;
							...
							me.markNote = { minfo: measure, morder: this.morder, start192: start192, duration192: duration192, pitch: pitch };
						}
						me.shrinkMeasures();
						me.reCalcContentSize();
					});
...
FretChordSheet.prototype.tilePianorollMark = function (left, top, width, height, lineWidth) {
	...
	if (this.markNote) {
		...
		for (var x = 0; x <= this.measures.length; x++) {
			...
			if (minfo)
				if (this.markNote.morder == x) {
					for (var y = 0; y < 5; y++) {

						var s = 4 - y;
						//console.log(x,y,s,this.markNote.pitch);
						if (this.markNote.pitch >= s * 12 && this.markNote.pitch < s * 12 + 12) {
							this.layerNotes.renderGroup(mx + this.margins.sheetLeft + this.options.measureHeader * 3 * this.tiler.tapSize
								, this.margins.pianorollTop + y * 3 * 12 * this.tiler.tapSize
								, (this.options.measureHeader + minfo.duration4 * 8 - this.options.measureHeader) * 3 * this.tiler.tapSize
								, 12 * 3 * this.tiler.tapSize
								, 'markNote', left, top, width, height, function (tg) {
									//console.log(tg);
									tg.layer.tileCircle(tg.g
										, tg.x + me.markNote.start192 / 6 * 3 * me.tiler.tapSize + 3 * me.tiler.tapSize / 2
										, tg.y + tg.h - (me.markNote.pitch % 12) * 3 * me.tiler.tapSize - +3 * me.tiler.tapSize / 2
