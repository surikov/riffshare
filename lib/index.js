...
FretChordSheet.prototype.tileNoteNatural = function (x, y, tg) {
	tg.layer.tilePath(tg.g, tg.x + (x * 3 + 0.5 + 0) * tg.layer.tapSize, tg.y + (y * 3 - 0.75 - 2) * tg.layer.tapSize
...
FretChordSheet.prototype.tileNoteFlat = function (x, y, tg) {
	tg.layer.tilePath(tg.g, tg.x + (x * 3  - 70) * tg.layer.tapSize
		, tg.y + (y * 3 - 310) * tg.layer.tapSize
		, 0.7 * tg.layer.tapSize //
	...
	for (var x = 0; x < this.measures.length; x++) {
		...
		this.layerOctaves.renderGroup(mx + this.margins.sheetLeft
			...
			, 'staffButtons' + x, left, top, width, height, function (tg) {
				...
				var _x = x;
				s = me.tileKnob(tg, 'rollKyesSel_' + x, 3 * me.tiler.tapSize, 48 * me.tiler.tapSize, 9 * me.tiler.tapSize, '' , function () {
					//console.log('test',_x);
					me.userActionRollKeys(_x);
				});
				s = me.tileKnob(tg, 'rollKyesSel2_' + x, 3 * me.tiler.tapSize, 90 * me.tiler.tapSize, 9 * me.tiler.tapSize, 'Sign/' + minfo.keys, function () {
					//console.log('test',_x);
					me.userActionRollKeys(_x);
				});
				...
				//if(me.keys[minfo.keys][0]==1){me.tileNoteSharp(9,10,tg);}
				if(me.keys[minfo.keys][0]==1)me.tileNoteSharp(12-9,9,tg);
				if(me.keys[minfo.keys][1]==1)me.tileNoteSharp(10-9,10,tg);
				if(me.keys[minfo.keys][2]==1)me.tileNoteSharp(15-9,11,tg);
				if(me.keys[minfo.keys][3]==1)me.tileNoteSharp(13-9,12,tg);
				if(me.keys[minfo.keys][4]==1)me.tileNoteSharp(11-9,13,tg);
				if(me.keys[minfo.keys][5]==1)me.tileNoteSharp(16-9,14,tg);
				if(me.keys[minfo.keys][6]==1)me.tileNoteSharp(14-9,15,tg);
				me.tileNoteFlat(12-1,11,tg);
				me.tileNoteFlat(14-1,11,tg);
				me.tileNoteFlat(12-1,11,tg);
				me.tileNoteFlat(12-1,11,tg);
				me.tileNoteFlat(12-1,11,tg);
				me.tileNoteFlat(12-1,11,tg);
											...
											for (var n = 0; n < chord.notes.length; n++) {
												...
												if (note)
													if (note.pitch >= 12 * (4 - y) && note.pitch < 12 * (5 - y)) {
														me.tilePianorollNoteLine(tg, note, beat.start192, x, me.trackInfo[c].shadow);
													}
											}
									}
								}
