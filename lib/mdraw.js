...
FretChordSheet.prototype.tileDrumRollSpot = function (left, top, width, height, layer, lineWidth) {
	var me = this;
	for (var i = 0; i < this.calcMeasureCount() + 1; i++) {
		layer.renderGroup(this.margins.sheetLeft + i * (this.options.measureLen + this.options.measureHeader) * 3 * layer.tapSize + this.options.measureHeader * 3 * layer.tapSize//
			, this.margins.drumsTop//
			, this.options.measureLen * 3 * layer.tapSize//
			, 8 * 3 * layer.tapSize//
...
FretChordSheet.prototype.clickedMeasureBeat = function () {
	...
	var remainX = xX - beatMeasure * this.options.measureLen - beatQuarter * 8;
	var r6 = 0;
	var l6 = 0;
	...
	console.log(xX,beatMeasure,beatQuarter);
	return { x6: r6, l6: l6 };
};
...
FretChordSheet.prototype.tileDrumRollGrid = function (left, top, width, height, layer, lineWidth) {
	...
	for (var i = 0; i < this.calcMeasureCount() + 1; i++) {
		...
		layer.renderGroup(this.margins.sheetLeft + i * (this.options.measureLen + this.options.measureHeader) * 3 * layer.tapSize//
			, this.margins.drumsTop//
			, (this.options.measureLen + this.options.measureHeader) * 3 * layer.tapSize//
			, 8 * 3 * layer.tapSize//
