...
FretChordSheet.prototype.tileMenu = function (left, top, width, height, layer, lineWidth) {
	...
	layer.renderGroup(0, 0, me.margins.sheetLeft, this.contentHeight, 'menu', left, top, width, height, function (tg) {
		me.tileRange(tg, 'btnFeel', 3 * me.tiler.tapSize, 1 * 3 * me.tiler.tapSize, 3 * me.tiler.tapSize, me.margins.sheetLeft - 6 * me.tiler.tapSize, 1 + me.options.feel, 4, 'Feel ' + me.feelNames[me.options.feel], function (v) {
							...
							if (me.options.feel == 2) {
								...
								if (me.options.feel == 1) {
									/*for (var nn = 1; nn < 5; nn++) {
										layer.tileRectangle(tg.g, tg.x + (8 * k + nn * 5 / 4) * 3 * layer.tapSize, tg.y
											, lineWidth * 0.5, 12 * 3 * layer.tapSize, me.colors.grid);
									}
									for (var nn = 1; nn < 4; nn++) {
										layer.tileRectangle(tg.g, tg.x + (8 * k +5 + nn * 4 / 5) * 3 * layer.tapSize, tg.y
										, lineWidth * 0.5, 12 * 3 * layer.tapSize, me.colors.grid);
									}*/
