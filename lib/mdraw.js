...
FretChordSheet.prototype.tileOctaveNumbers = function (sz,ww, left, top, width, height, layer, lineWidth) {
	var me = this;
	console.log('ww',ww,sz);
	for (var i = 0; i < 5; i++) {
		layer.renderGroup(ww //-sz/50 //5 * 3 * layer.tapSize//
			, i * 5 * 3 * layer.tapSize//
			, 9 * 3 * layer.tapSize//
			, sz//
...
FretChordSheet.prototype.tileBarNumbers = function (sz, left, top, width, height, layer, lineWidth) {
	var me = this;
	for (var i = 0; i < this.options.measureCount; i++) {
		layer.renderGroup(this.margins.sheetLeft + i * this.options.measureLen * 3 * layer.tapSize//
			, 0//
			, this.options.measureLen * 3 * layer.tapSize//
			, sz//
...
FretChordSheet.prototype.tileDrumGrid = function (left, top, width, height, layer, lineWidth) {
	var me = this;
	for (var i = 0; i < this.options.measureCount; i++) {
		...
		layer.renderGroup(this.margins.sheetLeft + i * this.options.measureLen * 3 * layer.tapSize//
			...
			, this.options.measureLen * 3 * layer.tapSize//
			, 8 * 3 * layer.tapSize//
			, 'drum' + i, left, top, width, height, function (tg) {

				for (var yy = 0; yy < 9; yy++) {
					layer.tileRectangle(tg.g, tg.x, tg.y + 3 * yy * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				}
				layer.tileRectangle(tg.g, tg.x + tg.w, tg.y, lineWidth, 8 * 3 * layer.tapSize, me.colors.staff);
			});
	}
};
FretChordSheet.prototype.tilFretGrid = function (left, top, width, height, layer, lineWidth) {
	var me = this;
	for (var i = 0; i < this.options.measureCount; i++) {
		//console.log(this,this.margins,this.margins.drumTop);
		layer.renderGroup(this.margins.sheetLeft + i * this.options.measureLen * 3 * layer.tapSize//
			...
			, 'fret' + i, left, top, width, height, function (tg) {
				...
				for (var yy = 0; yy < 6; yy++) {
					layer.tileRectangle(tg.g, tg.x, tg.y + 3 * yy * layer.tapSize + 1.5 * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				}
	...
	for (var i = 0; i < this.options.measureCount; i++) {
		layer.renderGroup(this.margins.sheetLeft + i * this.options.measureLen * 3 * layer.tapSize//
			...
			, 'stff' + i, left, top, width, height, function (tg) {
				...
				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + 6 * 0) * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + 6 * 1) * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + 6 * 2) * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + 6 * 3) * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + 6 * 4) * layer.tapSize, tg.w, lineWidth, me.colors.staff);

				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + sk + 6 * 0) * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + sk + 6 * 1) * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + sk + 6 * 2) * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + sk + 6 * 3) * layer.tapSize, tg.w, lineWidth, me.colors.staff);
				layer.tileRectangle(tg.g, tg.x, tg.y + (37.5 + sk + 6 * 4) * layer.tapSize, tg.w, lineWidth, me.colors.staff);

				layer.tileRectangle(tg.g, tg.x + tg.w, tg.y + (37.5 + 6 * 0) * layer.tapSize, lineWidth, 75 * layer.tapSize, me.colors.staff);
