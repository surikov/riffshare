...
FretChordSheet.prototype.tilePianoSpot = function (left, top, width, height, layer, lineWidth) {
	...
	layer.renderGroup(this.margins.sheetLeft //
		...
		, this.options.measureCount*this.options.measureLen * 3 * layer.tapSize//
			...
			layer.renderGroup(this.margins.sheetLeft + i * this.options.measureLen * 3 * layer.tapSize//
				...
				, 'pianoNotes' + i + 'x' + n, left, top, width, height, function (tg) {
					for(var t=0;t<me.tones.length;t++){
						var tx=me.tones[t].beat192/6*3*layer.tapSize+me.margins.sheetLeft;
						var ty=(5*12-me.tones[t].pitch)*3*layer.tapSize+me.margins.pianorollTop;
						//console.log(t,tx,ty);
						if(tx>=tg.x 
							&& tx<=tg.x+tg.w
							&& ty>=tg.y
							...
							tg.layer.tileLine(tg.g
								...
								, ty+1.5*layer.tapSize
								, me.colors.base, 3*layer.tapSize);
						}
					}
				});
		}
		...
		layer.renderGroup(this.margins.sheetLeft + i * this.options.measureLen * 3 * layer.tapSize//
			...
			, 'drpnts' + i, left, top, width, height, function (tg) {
				...
				for(var t=0;t<me.drums.length;t++){
					var tx=me.drums[t].beat192/6*3*layer.tapSize+me.margins.sheetLeft;
