					...
					for (var t = 0; t < me.tones.length; t++) {
						...
						if (tx >= tg.x
							...
							tg.layer.tileLine(tg.g
								...
								tg.layer.tileText(tg.g,tx + 1.3 * layer.tapSize,ty + 2.5 * layer.tapSize
...
var ty = 6* 3 * layer.tapSize + me.margins.fretTop-(6-st)* 3 * layer.tapSize;
					if (tx >= tg.x
						&& tx <= tg.x + tg.w
						&& ty >= tg.y
						&& ty <= tg.y + tg.h
						...
						tg.layer.tileLine(tg.g
							...
							, ty + 1.5 * layer.tapSize + me.tones[t].shift * 3 * layer.tapSize
							, me.colors.base, 2.9 * layer.tapSize);
							var frnum=me.strings[st]-me.tones[t].pitch;
							tg.layer.tileText(tg.g,tx + 1.3 * layer.tapSize,ty + 2.5 * layer.tapSize
