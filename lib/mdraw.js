...
FretChordSheet.prototype.tileSongName = function (left, top, width, height,  layer, lineWidth) {
...
FretChordSheet.prototype.tileTest = function (left, top, width, height,  layer, lineWidth) {
	...
	layer.renderGroup(10 * layer.tapSize, 10 * layer.tapSize, 50 * layer.tapSize, 50 * layer.tapSize, 'tstxt2', left, top, width, height, function (tg) {
		//layer.tileRectangle(tg.g, tg.x, tg.y, tg.w, tg.h, 'rgba(256,0,0,0.25)');
		layer.tileText(tg.g, tg.x, tg.y+tg.h, 36 * layer.tapSize, 'Test text', me.colors.barCounter);
		//console.log('test');
	});
	/*
...
FretChordSheet.prototype.tileStaff = function (left, top, width, height,  layer, lineWidth) {
	...
	if (tg) {
		...
		this.tileNoteSingle16down(2, 4, tg);
		this.tileNoteSingle16up(4, 10, tg);
		this.tileNoteSingle8down(8, 4, tg);
		this.tileNoteSingle8up(10, 10, tg);
		...
		layer.tileRectangle(tg.g, this.margins.sheetX, this.margins.sheetY + 6 * 0 * layer.tapSize, this.contentWidth, lineWidth, this.colors.base);
		layer.tileRectangle(tg.g, this.margins.sheetX, this.margins.sheetY + 6 * 1 * layer.tapSize, this.contentWidth, lineWidth, this.colors.base);
		layer.tileRectangle(tg.g, this.margins.sheetX, this.margins.sheetY + 6 * 2 * layer.tapSize, this.contentWidth, lineWidth, this.colors.base);
		layer.tileRectangle(tg.g, this.margins.sheetX, this.margins.sheetY + 6 * 3 * layer.tapSize, this.contentWidth, lineWidth, this.colors.base);
		layer.tileRectangle(tg.g, this.margins.sheetX, this.margins.sheetY + 6 * 4 * layer.tapSize, this.contentWidth, lineWidth, this.colors.base);
...
FretChordSheet.prototype.tileBars = function (left, top, width, height,  layer, lineWidth) {
	for (var i = 0; i < this.barCount; i++) {
		var xx = this.margins.sheetX + i * 3 * this.barSize * layer.tapSize;
		var tg = layer.tileGroup(0, 0, this.contentWidth, this.contentHeight, 'brLine' + i, left, top, width, height);
		if (tg) {
			layer.tileRectangle(tg.g, xx, this.margins.sheetY, lineWidth, 2 * 3 * 4 * layer.tapSize, this.colors.base);
		}
	}
}
FretChordSheet.prototype.tileMeasureGrid = function (left, top, width, height,  layer, lineWidth, n) {
	var xx = this.margins.sheetX + n * 3 * this.barSize * layer.tapSize;
...
FretChordSheet.prototype.tileNoteHead = function (x, y, tg) {
	tg.layer.tilePath(tg.g, tg.x + this.margins.sheetX + (x * 3 + 0.5) * tg.layer.tapSize, tg.y + (y * 3 - 0.5) * tg.layer.tapSize
...
FretChordSheet.prototype.tileNoteSharp = function (x, y, tg) {
	tg.layer.tilePath(tg.g, tg.x + this.margins.sheetX + (x * 3 + 0.5 - 38) * tg.layer.tapSize
...
FretChordSheet.prototype.tileNoteFlat = function (x, y, tg) {
	tg.layer.tilePath(tg.g, tg.x + this.margins.sheetX + (x * 3 + 0.5 - 52) * tg.layer.tapSize, tg.y + (y * 3 - 0.5 - 219) * tg.layer.tapSize
...
FretChordSheet.prototype.tileNoteNatural = function (x, y, tg) {
	tg.layer.tilePath(tg.g, tg.x + this.margins.sheetX + (x * 3 + 0.5 - 4) * tg.layer.tapSize, tg.y + (y * 3 - 0.5 - 2) * tg.layer.tapSize
...
FretChordSheet.prototype.tileNoteStemUp = function (x, y, tg) { //, yShift, hShift) {
	//var dy = yShift || 0;
	//var dh = hShift || 0;
	tg.layer.tileRectangle(tg.g, tg.x + this.margins.sheetX + (x * 3 + 4.05) * tg.layer.tapSize
...
FretChordSheet.prototype.tileNoteStemDown = function (x, y, tg) {
	tg.layer.tileRectangle(tg.g, tg.x + this.margins.sheetX + (x * 3 - 0.7) * tg.layer.tapSize, tg.y + (y * 3 + 2.4) * tg.layer.tapSize
...
FretChordSheet.prototype.tileNoteSingle8up = function (x, y, tg) {
	this.tileNoteHead(x, y, tg);
	this.tileNoteStemUp(x, y, tg);
	tg.layer.tilePath(tg.g, tg.x + this.margins.sheetX + (x * 3 + 4.2) * tg.layer.tapSize, tg.y + (y * 3 - 9.6 - 9) * tg.layer.tapSize
...
FretChordSheet.prototype.tileNoteSingle8down = function (x, y, tg) {
	this.tileNoteHead(x, y, tg);
	this.tileNoteStemDown(x, y, tg);
	tg.layer.tilePath(tg.g, tg.x + this.margins.sheetX + (x * 3 - 0.5) * tg.layer.tapSize, tg.y + (y * 3 + 12.8 + 9.4) * tg.layer.tapSize
	, 0.05 * tg.layer.tapSize //
...
FretChordSheet.prototype.tileNoteSingle16down = function (x, y, tg) {
	...
	, 0.05 * tg.layer.tapSize //
		...
		, this.colors.base);
		tg.layer.tilePath(tg.g, tg.x + this.margins.sheetX + (x * 3 - 0.5) * tg.layer.tapSize, tg.y + (y * 3 + 10.8 + 8.4) * tg.layer.tapSize
		, 0.05 * tg.layer.tapSize //
		, "M0,0c12.6003,3.13153 8.52623,-13.80134 11.38796,-21.63203c2.73069,-23.02648 17.77757,-41.09454 32.75498,-57.22844c18.58194,-20.27957 38.31331,-41.78204 44.35311,-69.87805c6.59531,-32.5975 -3.94836,-65.94508 -17.63124,-95.15867c-0.99081,-9.36267 -17.72272,-22.90831 -9.75105,-5.98947c13.4229,28.94262 24.51194,62.27485 15.34996,94.32426c-8.72487,31.97927 -43.57272,61.42695 -75.94619,66.21582l-0.51753,89.34658z" //
		, this.colors.base);
}
FretChordSheet.prototype.tileNoteSingle16up = function (x, y, tg) {
	...
	, 0.05 * tg.layer.tapSize //
		...
		, this.colors.base);
		tg.layer.tilePath(tg.g, tg.x + this.margins.sheetX + (x * 3 + 4.2) * tg.layer.tapSize, tg.y + (y * 3 - 7.6 - 8) * tg.layer.tapSize
		, 0.05 * tg.layer.tapSize //
		, "M0,0c12.6003,-3.05094 8.52623,13.44614 11.38796,21.0753c2.73069,22.43386 17.77757,40.03692 32.75498,55.75559c18.58194,19.75765 38.31331,40.70672 44.35311,68.07965c6.59531,31.75856 -3.94836,64.2479 -17.63124,92.70964c-0.99081,9.12171 -17.72272,22.31873 -9.75105,5.83532c13.4229,-28.19774 24.51194,-60.67212 15.34996,-91.8967c-8.72487,-31.15624 -43.57272,-59.84605 -75.94619,-64.51167l-0.51753,-87.04713z" //
		, this.colors.base);
}
...
FretChordSheet.prototype.tileNoteGroupUp = function (group, tg) {
	...
	for (var i = 0; i < group.length - 1; i++) {
		if (group[i].duration32 < 4) {
			...
			var bx1 = tg.x + (10.3 + 3 * sx1) * tg.layer.tapSize;
			var by1 = tg.y + (3 * sy1 + - 9.7 - 9 - dy * 3) * tg.layer.tapSize;
			var bx2 = tg.x + (10.8 + 3 * sx2 - 0.65) * tg.layer.tapSize;
	...
	var bx1 = tg.x + (10.3 + 3 * x1) * tg.layer.tapSize;
	var by1 = tg.y + (3 * y1 - 9.7 - 9 - dy * 3) * tg.layer.tapSize;
	var bx2 = tg.x + (10.1 + 3 * x2) * tg.layer.tapSize;
...
FretChordSheet.prototype.tileNoteGroupDown = function (group, tg) {
	...
	for (var i = 0; i < group.length; i++) {
		...
		tg.layer.tileRectangle(tg.g, tg.x + this.margins.sheetX + (x * 3 - 0.7) * tg.layer.tapSize
	...
	for (var i = 0; i < group.length - 1; i++) {
		if (group[i].duration32 < 4) {
			...
			var sx2 = group[i + 1].step32;
			var sy2 = group[0].chord[group[0].chord.length - 1] + r * (group[i + 1].step32 - group[0].step32);
			var bx1 = tg.x + (6.3 + 3 * sx1 - 0.65) * tg.layer.tapSize;
			var by1 = tg.y + (3 * sy1 + 20.4 - 3 - dy * 3) * tg.layer.tapSize;
			var bx2 = tg.x + (6.1 + 3 * sx2 - 0.65) * tg.layer.tapSize;
	...
	var bx1 = tg.x + (6.3 + 3 * x1 - 0.65) * tg.layer.tapSize;
	var by1 = tg.y + (3 * y1 + 20.4 - dy * 3) * tg.layer.tapSize;
	var bx2 = tg.x + (6.1 + 3 * x2 - 0.65) * tg.layer.tapSize;
