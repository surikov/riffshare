...
function SongInfo(){
	...
	sng.addHugeTiles=function(me,left, top, width, height){
		for(var i=102;i<sng.duration16();i=i+202){
			var id='hs'+i;
			if (!me.childExists(id, me.layerHugeBack)) {
				tileSpot(me,me.tapSize*i,me.tapSize*1,me.tapSize*200,me.tapSize*(sng.tracks.length*30+128),me.layerHugeFront,id);
			}
		}
	}
	sng.addLargeTiles=function(me,left, top, width, height){
		for(var i=0;i<sng.tracks.length;i++){
			...
			if (!me.childExists(id, me.layerHugeBack)) {
				...
				tileSpot(me,me.tapSize*1,me.tapSize*(1+i*30),me.tapSize*100,me.tapSize*29,me.layerLargeFront,id);
			}
			id='line'+i+'label';
			if (!me.childExists(id, me.layerHugeBack)) {
				tileSpot(me,me.tapSize*102,me.tapSize*(1+i*30),me.tapSize*(16*300-102),me.tapSize*10,me.layerLargeFront,id);
			}
		}
		var cnt=0;
		for(var i=102;i<sng.duration16();i=i+16){
			var id='m'+cnt;
			if (!me.childExists(id, me.layerHugeBack)) {
				tileSpot(me,me.tapSize*i,me.tapSize*(2+sng.tracks.length*30),me.tapSize*15,me.tapSize*128,me.layerLargeFront,id);
			}
			cnt++;
		}
		cnt=0;
		for(var i=0;i<128;i=i+12){
			var id='octave'+cnt;
			if (!me.childExists(id, me.layerHugeBack)) {
				tileSpot(me,me.tapSize*102,me.tapSize*(12+i+2+sng.tracks.length*30),me.tapSize*(16*300-102),me.tapSize*11,me.layerLargeFront,id);
			}
			cnt++;
		}
	}
