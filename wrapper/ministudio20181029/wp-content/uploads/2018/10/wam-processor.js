...
class WAMProcessor extends AudioWorkletProcessor
{
  ...
  addEvents (events) {
    for (let i = 0;  i < events.length; i++)
      if (events[i].type == "note") {
        let e = events[i].data.midi;
        this.onmidi(e[0], e[1], e[2]);
      }
  }
  ...
  onparam (key, value) {
    if (typeof key === "string") {
      if (key == "bypass") this.bypass = value;
  ...
  onmsg (verb, prop, data) {
    // amped
    if (prop == "WAM" && verb == "add")
      AudioWorkletGlobalScope.AmpedDSP.devices[data] = this;
    ...
    if (this.bypass) {
      for (var i=0; i<this.numOutputs; i++) {
        ...
        for (var c=0; c<numChannels; c++) {
          ...
          if (i < this.numInputs && c < this.numInChannels) {
