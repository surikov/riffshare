...
RiffShare2D.prototype.tilePlaceHolder = function (x,y,w,h,id,layer, left, top, width, height) {
	...
	if(g){
		...
		this.tileRectangle(g,x,y,w,h,'rgba(255,0,0,0.25)','#f00',s/30);
		this.tileText(g,x,y,s/3,id);
		this.tileLine(g,x,y+h,x+w,y,'#0f0',s/30);
		this.tileCircle(g,x+w,y+h,s/30,'#00f');
		this.addSpot(id,x+w-s/30,y+h-s/30,2*s/30,2*s/30,function(){console.log(id,'clicked');});
	}
};
...
RiffShare2D.prototype.tileText = function (g,x,y,n,m,f,s,t) {
	...
	if(f){rect.setAttributeNS(null, 'fill', f);}
	if(s){rect.setAttributeNS(null, 'stroke', s);}
	if(t){rect.setAttributeNS(null, 'stroke-width', t);}
	txt.innerHTML = m;
	g.appendChild(txt);
};
...
RiffShare2D.prototype.tileCircle = function (g,x,y,r,f,s,t) {
	...
	if(f){circle.setAttributeNS(null, 'fill', f);}
	if(s){circle.setAttributeNS(null, 'stroke', s);}
	if(t){circle.setAttributeNS(null, 'stroke-width', t);}
...
RiffShare2D.prototype.tileLine = function (g,x1,y1,x2,y2,s,t) {
	...
	if(s){line.setAttributeNS(null, 'stroke', s);}
	if(t){line.setAttributeNS(null, 'stroke-width', t);}
