RiffShare2D.prototype.addHugeTiles = function (xx, yy, ww, hh, detailRatio) {
	...
	this.tileHugeSongOptions(this.hugetitles, xx, yy, ww, hh, detailRatio);
	...
	this.tilePianoRollMenu(this.hugetitles, xx, yy, ww, hh, detailRatio);
...
RiffShare2D.prototype.tilePianoRollMenu = function (layer, left, top, width, height, ratio) {
	...
	if (g) {
		...
		if (this.currentSong.hideRoll) {
			this.addHollowButton('btRlHd', 'Show Pianoroll', g, x/2-h/2, y, h,  function () {
				riffShare2d.redoTogglePianoroll();
			});
		} else {
			this.addSimpleButton('btRlHd', 'Hide Pianoroll', g, x/2-h/2, y, h, function () {
				riffShare2d.redoTogglePianoroll();
			});
		}
		...
		*/
		this.addSimpleButton('btNCh', 'New channel', g, x/2-h/2 , y-  h, h,  function () {
			riffShare2d.promptAddNewChannel();
		});
	}
};
RiffShare2D.prototype.tileHugeSongOptions = function (layer, left, top, width, height, ratio) {
	...
	if (g) {
		 ...
		 */
		this.addZoomAnchor('btMenSng', '', g, x/2-h/2, y, h, 0.9 * this.minZoomLarge);
	}
};
RiffShare2D.prototype.tileHugeSongTitle = function (layer, left, top, width, height, ratio) {
	...
	if (g) {
		...
		this.addSimpleButton(id, 'Play', g, x, y, h, function () {
			console.log('play pressed');
		});
		this.tileText(g, x + 1 * (1.2 * h), y + h, h / 1, this.currentSong.name, this.colorBase);
	}
