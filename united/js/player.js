...
FretChordSheet.prototype.waplayer = function () {
	if (!(this.player)) {
		...
		if (binfo.start192 >= 12 * beat16 && binfo.start192 < 12 * (beat16 + 1)) {
			...
			for (var d = 0; d < binfo.drums.length; d++) {
				if (binfo.drums[d]) {
					...
					var drumData=this.cachedDrum(this.drumInfo[d]);
...
FretChordSheet.prototype.cachedDrum = function (drum,pitch) {
	if (this.subSamples[drum.id+8]) {
		//console.log(drum,this.subSamples[drum.id+8]);
		var key = '_drum_' +this.player.loader.drumKeys()[this.subSamples[drum.id+8]-1];
		if (this.player.loader.loaded([ key])) {
			var sound=window[ key];
			return {sound:sound,pitch:sound.zones[0].keyRangeLow};
		}else{
			var info = this.player.loader.drumInfo(this.subSamples[drum.id+8]-1);
			this.player.loader.startLoad(this.audioContext, info.url, info.variable);
		}
	}
	//
	return {sound:drum.sound,pitch:drum.sound.zones[0].keyRangeLow};
};